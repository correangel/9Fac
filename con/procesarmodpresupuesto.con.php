...
$datosPresupuesto["IdPresupuesto"] = $_POST["IdPresupuesto"];
...
$sql = "UPDATE ges_presupuestos SET SeriePresupuesto=%SeriePresupuesto%,".
	"NPresupuesto=%NPresupuesto%,FechaPresupuesto=%FechaPresupuesto%,IdCliente=%IdCliente%,
		    ImporteNeto=%ImporteNeto%,IvaImporte=%IvaImporte%,TotalImporte=%TotalImporte%,
		     SerieNumeroPresupuesto=%SerieNumeroPresupuesto%,
			NombreComercial=%NombreComercial%,Direccion=%Direccion%,Poblacion=%Poblacion%,CIF=%CIF%,CodigoPostal=%CodigoPostal%, ".
	" ObraRealizada=%ObraRealizada% WHERE IdPresupuesto=%IdPresupuesto%  ";
...
query("DELETE FROM ges_presupuestos_det WHERE IdPresupuesto='$IdPresupuesto'");
