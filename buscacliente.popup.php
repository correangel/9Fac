<?php

define ("BUSCACLIENTEPOPUP",1);

include("tool.php");

switch($modo){
	case "JSONBuscaCliente":
		$cadena = $_REQUEST["cadena"];
		$cadena_s = CleanRealMysql($cadena);
				
		$otrasCondiciones = "";
		$otrasCondiciones .= " OR ( Localidad LIKE '%$cadena_s%'  )";
		$otrasCondiciones .= " OR ( NombreLegal LIKE '%$cadena_s%'  )";
		$otrasCondiciones .= " OR ( Direccion LIKE '%$cadena_s%'  )";
		$otrasCondiciones .= " OR ( Contacto LIKE '%$cadena_s%'  )";
		$otrasCondiciones .= " OR ( NumeroFiscal LIKE '%$cadena_s%'  )";
		$otrasCondiciones .= " OR ( Email LIKE '%$cadena_s%'  )";
		$otrasCondiciones .= " OR ( IdCliente = '%$cadena_s%'  )";
						
		$sql = "SELECT * FROM ges_clientes WHERE (NombreComercial LIKE '%$cadena_s%') $otrasCondiciones ORDER BY NombreComercial ASC"; 
		$res = query($sql);
		if(!$res){
			echo "0";
			exit();	
		}
		
		$rows = array();	
	
		$num = 0;
		while($row = Row($res) ){			
			$rows[$num++] = $row;	
		} 
		
		$json = new Services_JSON();
		$output = $json->encode($rows);	
		echo $output;					
				
				
		exit();	
		
		break;	
}

StartXul();


?>
<groupbox flex="1"> 
<label value="Nombre/Cod/Otro:"/>
<hbox>
<textbox flex="1" id="cadenaBusqueda"  onchange="lanzaBusqueda()" />
<toolbarbutton  image="img/busca1.gif" oncommand="lanzaBusqueda()" />
</hbox>
<listbox flex="1" id="listaClientes">
</listbox>
</groupbox>

<script><![CDATA[

 var lista = "listaClientes";


 document.gClonedListbox = false;
 
 function Startup() {
      document.gClonedListbox = $(lista).cloneNode(true);
	  $("cadenaBusqueda").focus();
 }

 function clearListbox() {
     var oldListbox = $(lista);         
     var newListbox = document.gClonedListbox.cloneNode(true);
     oldListbox.parentNode.replaceChild( newListbox,oldListbox);
 }
 
 function lanzaBusqueda(){
 	clearListbox();
	var cadenaBusqueda = $("cadenaBusqueda").value;

	var obj = new XMLHttpRequest();
	var url = "buscacliente.popup.php";
	var data = "&modo=JSONBuscaCliente&cadena=" + encodeURIComponent(cadenaBusqueda);

	obj.open("POST",url,false);
	obj.setRequestHeader('Content-Type','application/x-www-form-urlencoded; charset=UTF-8');
	obj.send(data);
	
	var text = obj.responseText;

	var obj =  null;

	try {
		obj = eval( "(" + text+ ")" );
	} catch(e){
		return alert("e:"+e+",info:"+ text);
	}
	
	var num = 0;

	for (idcliente in obj){
	   cliente = obj[idcliente] ;
	   //num = num + " " + client["IdCliente"]; 			
	   //num = num + " " + client["NombreComercial"];
	   
	   AddClientToList(cliente["IdCliente"], cliente["NombreComercial"]);
	}
	
	if (!num){	
		AddMarcaVacio();
	}
			
 }
 
 
 function AddMarcaVacio(){
 	var xitem = document.createElement("listitem");
 	var xcell = document.createElement("listcell");
 	xcell.setAttribute("label","No se encontraron datos");
 	xitem.appendChild(xcell);
 	 	
 	$(lista).appendChild( xitem ); 		
 } 
 
 
 function AddClientToList(IdCliente, NombreCliente){
 	//IdCliente =ParseInt(IdCliente);
 
 	var xitem = document.createElement("listitem");
 	var xcell = document.createElement("listcell");
 	xcell.setAttribute("label",NombreCliente);
	xcell.setAttribute("flex","1");
	xcell.setAttribute("style","min-width: 26em");

 	var xcell1 = document.createElement("listcell");
 	xcell1.setAttribute("label",IdCliente);
 	xcell1.setAttribute("flex","0"); 	 	
 	
 	xitem.appendChild(xcell1);
 	xitem.appendChild(xcell);
 	
 	xitem.setAttribute("onclick","InvocarCliente('"+IdCliente+"')");
 	
 	$(lista).appendChild( xitem ); 		 	
 }
 
function InvocarCliente(IdCliente){
	window.parent.UsarCliente(IdCliente);
	window.close();
}


 Startup();

]]></script>
</window>