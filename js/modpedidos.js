...
function VaciarListaProductos(){
 ...
 function Startup() {
     ...
     $("datoFecha").value = Pedido.fecha;
     $("datoNumpedido").value = Pedido.numeropedido;
 }
...
function ActualizarTotalesProductos(){
	...
	//baseimponible,importeiva,totalpedido

	$("totalpedido").value 	= formatoBonito(megaTotalConIva);
	$("totalpedido").setAttribute("valor",megaTotalConIva);
...
function EnviarPedido(){
	...
	var totalPedido = $("totalpedido").getAttribute("valor");
	...
	cadenaEnvio += urlVar("totalpedido",totalPedido);
	...
	cadenaEnvio += urlVar("SeriePedido",Pedido.serie);
	cadenaEnvio += urlVar("NPedido",Pedido.numero);

	cadenaEnvio += urlVar("IdCliente",Pedido.IdCliente);



	//datoNumpedido, datoFecha
	var valorCampo = $("datoNumpedido").value;
	cadenaEnvio += urlVar("SerieNumeroPedido",valorCampo);

	var valorCampo = $("datoFecha").value;
	cadenaEnvio += urlVar("FechaPedido",valorCampo);

	var obj = new XMLHttpRequest();
	var url = moduloAjax;

	cadenaEnvio +="&modo=EnviarPedido";
	...
	InterpretarDatosCreacionPedido(obj.responseText);

}
/* ------------------------------------ */

function InterpretarDatosCreacionPedido(rawtext){
...
function quitarElementoPedido(elemento){
...
function AbrirVentanaNumpedido(){
	var fichero = "modificarserie.popup.php?" +
		"&serie=" + encodeURIComponent(Pedido.serie) +
		"&numero=" + encodeURIComponent(Pedido.numero);

	AbrirVentanaYBuscar(fichero,300);
}


function UsarFormatoNumpedido(DescripcionFormato, serie, numero){
	//datoNumpedido
	var xcode = $("datoNumpedido");
	xcode.value = DescripcionFormato;

	Pedido.serie = serie;
	Pedido.numero = numero;
	Pedido.numeropedido = DescripcionFormato;
}


/*--------------------------------*/

 function Accion_BuscarPedido(){

	var fichero = "buscapedido.popup.php?";
 ...
 function LimpiarCamposPedido(){
 	...
 	var campos = new String("baseimponible,importeiva,totalpedido,CodCliente,codigo,referencia,descripcion,unid,precio,dto,impuesto,NombreComercial,Direccion,Poblacion,CIF,CodigoPostal,datoNumpedido,datoFecha,obraRealizada");
...
function CargarSiguientesDatosDePedido(){
	var url = moduloAjax;
	var data = "&modo=IniciarSiguientePedido";

	ajaxCarga.open("POST",url,true);
	ajaxCarga.onreadystatechange = IniciarSiguietePedido;
	ajaxCarga.setRequestHeader('Content-Type','application/x-www-form-urlencoded; charset=UTF-8');
	ajaxCarga.send(data)
}

function IniciarSiguietePedido(){
	ocupado = 0;
	if (ajaxCarga.readyState==4) {
		var rawtext = ajaxCarga.responseText;
		if (rawtext=="ERROR")		return;

		InterpretarDatosSiguientePedido(rawtext);
	}
}

function InterpretarDatosSiguientePedido(rawtext){
	var obj;
	if (!rawtext)	return;
	...
	try {
        ...
        setDatoURL2("datoNumpedido","expresionNumeroPedido",obj);


        Pedido.fecha = fecha;
        Pedido.numeropedido = obj["expresionNumeroPedido"];
        Pedido.numero = obj["NPedido"];
        Pedido.serie = obj["Serie"];

		console.log("[InterpretarDatosSiguientePedido]pedido:");
		console.dir( Pedido);
}


 //Solicitud para cargar una pedido en concreto
 function UsarPedido(IdPedido){
	
	//Modo modificacion
	$("cajaModoModificacion").setAttribute("collapsed","false");
	$("cajaModoAlta").setAttribute("collapsed","true");

 	AutoActualizarPedido( IdPedido );
 }


function AutoActualizarPedido(id){

	var url = moduloAjax + "?modo=JSONCargarPedido&id="+id;

	ajaxCarga.open("GET",url,true);
	ajaxCarga.onreadystatechange = CargarPedido;
	ajaxCarga.send(null)
...
function CargarPedido(){
	...
	if (ajaxCarga.readyState==4) {
		...
		InterpretarDatosPedido(rawtext);
	}
}


function InterpretarDatosPedido(rawtext){
	if (!rawtext)	return;
	...
	setDatoURL2("totalpedido","TotalImporte",obj);
	...
	setDatoURL2("datoNumpedido","NPedido",obj);
	setDatoURL2_fecha("datoFecha","FechaPedido",obj);

	setDatoURL2("obraRealizada","ObraRealizada",obj);

	Pedido.IdPedido = obj["IdPedido"];
	Pedido.serie = obj["SeriePedido"];
	Pedido.numero = obj["NPedido"];
	Pedido.IdCliente = obj["IdPedido"];

	//TODO: cargar lineas de pedido
	AutoActualizarLineasPedido( obj.IdPedido );

	//No nos fiamos de los datos que vienen de la base de datos, asi que actualizamos 
	ActualizarTotalesProductos();
}

function AutoActualizarLineasPedido(id){

	var url = moduloAjax + "?modo=JSONCargarLineasPedido&id="+id;

	ajaxCarga.open("GET",url,true);
	ajaxCarga.onreadystatechange = CargarLineasPedido;
	ajaxCarga.send(null)
...
function CargarLineasPedido(){
	...
	if (ajaxCarga.readyState==4) {
		...
		InterpretarLineasPedido(rawtext);
	}
}

function InterpretarLineasPedido(rawtext){

	function ActualizaCamposProductoDesdeLineaPedido(obj){
		$("codigo").value = obj["IdProducto"];
		$("referencia").value = obj["Referencia"];
	...
	for( t in obj){
		...
		ActualizaCamposProductoDesdeLineaPedido(obj[t]);
...
function VentanaImprimirPedido(){
   //   VerObj(Pedido);
        AbrirVentanaImprimir("moddocumento.php?modo=pedido&id="+Pedido.IdPedido);
}
...
function GuardarCambios(){
	...
	cadenaEnvio += "&IdPedido="+ Pedido.IdPedido;


	var obj = new XMLHttpRequest();
	var url = moduloAjax;

	cadenaEnvio +="&modo=ModificarPedido";
	...
	InterpretarDatosModPedido(obj.responseText);

}
 

function InterpretarDatosModPedido(rawtext){
	...
	try {
		...
		alert("Pedido modificada con exito");
		UsarPedido( obj["IdPedido"] );
