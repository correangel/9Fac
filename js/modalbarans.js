...
var moduloAjax ="modalbaran.php";

var esAlta = true;

var Albaran = new Object();
Albaran.IdAlbaran = 0;
...
function VaciarListaProductos(){
 ...
 function Startup() {
     ...
     $("datoFecha").value = Albaran.fecha;
     $("datoNumalbaran").value = Albaran.numeroalbaran;
 }
...
function ActualizarTotalesProductos(){
	...
	//baseimponible,importeiva,totalalbaran

	$("totalalbaran").value 	= formatoBonito(megaTotalConIva);
	$("totalalbaran").setAttribute("valor",megaTotalConIva);
...
function BorrarTodoIniciarSiguiente(){
	BorrarTodo();
	CargarSiguientesDatosDeAlbaran();
}
...
function ActualizarDatosCliente(obj){
	Albaran.IdCliente = obj["IdCliente"];
...
function ActualizarDatosProducto(obj){
	...
	//codigo,descripcion,unid,precio/unid,dto,impuesto
	 /*IdProducto   	  IdMarca   	  IdProdBase   	  Referencia
	  	  CodigoBarras   	  CosteSinIVA   	  Impuesto
	  	    StockIlimitado   	  RefProvHab   	  IdProvHab
	  	     IdTallaje   	  IdTalla   	  IdNumeroZapato
	  	      IdColor   	  IdFamilia   	  IdSubFamilia
	  	       Eliminado   	  IdProdIdioma   	  IdProdBase
	  	       	  IdIdioma   	  Nombre   	  Descripcion
	  	       	  	  Eliminado*/
...
function EnviarAlbaran(){
	...
	var totalAlbaran = $("totalalbaran").getAttribute("valor");
	...
	cadenaEnvio += urlVar("totalalbaran",totalAlbaran);
	...
	cadenaEnvio += urlVar("SerieAlbaran",Albaran.serie);
	cadenaEnvio += urlVar("NAlbaran",Albaran.numero);

	cadenaEnvio += urlVar("IdCliente",Albaran.IdCliente);



	//datoNumalbaran, datoFecha
	var valorCampo = $("datoNumalbaran").value;
	cadenaEnvio += urlVar("SerieNumeroAlbaran",valorCampo);

	var valorCampo = $("datoFecha").value;
	cadenaEnvio += urlVar("FechaAlbaran",valorCampo);

	var obj = new XMLHttpRequest();
	var url = moduloAjax;

	cadenaEnvio +="&modo=EnviarAlbaran";
	...
	InterpretarDatosCreacionAlbaran(obj.responseText);

}

function InterpretarDatosCreacionAlbaran(rawtext){
...
function quitarElementoAlbaran(elemento){
...
function AbrirVentanaNumalbaran(){
	var fichero = "modificarserie.popup.php?" +
		"&serie=" + encodeURIComponent(Albaran.serie) +
		"&numero=" + encodeURIComponent(Albaran.numero);

	AbrirVentanaYBuscar(fichero,300);
}


function UsarFormatoNumalbaran(DescripcionFormato, serie, numero){
	//datoNumalbaran
	var xcode = $("datoNumalbaran");
	xcode.value = DescripcionFormato;

	Albaran.serie = serie;
	Albaran.numero = numero;
	Albaran.numeroalbaran = DescripcionFormato;
}


/*--------------------------------*/

 function Accion_BuscarAlbaran(){

	var fichero = "buscaalbaran.popup.php?";
 ...
 function LimpiarCamposAlbaran(){

 	var campos = new String("baseimponible,importeiva,totalalbaran,CodCliente,codigo,referencia,descripcion,unid,precio,dto,impuesto,NombreComercial,Direccion,Poblacion,CIF,CodigoPostal,datoNumalbaran,datoFecha,obraRealizada");
...
function CargarSiguientesDatosDeAlbaran(){
	var url = moduloAjax;
	var data = "&modo=IniciarSiguienteAlbaran";

	ajaxCarga.open("POST",url,true);
	ajaxCarga.onreadystatechange = IniciarSiguieteAlbaran;
	ajaxCarga.setRequestHeader('Content-Type','application/x-www-form-urlencoded; charset=UTF-8');
	ajaxCarga.send(data)
}

function IniciarSiguieteAlbaran(){
	ocupado = 0;
	if (ajaxCarga.readyState==4) {
		var rawtext = ajaxCarga.responseText;
		if (rawtext=="ERROR")		return;

		InterpretarDatosSiguienteAlbaran(rawtext);
	}
}

function InterpretarDatosSiguienteAlbaran(rawtext){
	var obj;
	if (!rawtext)	return;
	...
	//VerObj(obj);
        //0 ->3 NAlbaran ->3 1 ->A SerieAlbaran ->A
        var hoy = new Date();

        var fecha = (hoy.getDate()) +"-"+(hoy.getMonth()+1) + "-"+hoy.getFullYear();
        $("datoFecha").value = fecha;

        setDatoURL2("datoNumalbaran","expresionNumeroAlbaran",obj);


        Albaran.fecha = fecha;
        Albaran.numeroalbaran = obj["expresionNumeroAlbaran"];
        Albaran.numero = obj["NAlbaran"];
        Albaran.serie = obj["Serie"];
	...
	//Albaran.IdProducto = obj.IdAlbaran;
}


 //Solicitud para cargar una albaran en concreto
 function UsarAlbaran(IdAlbaran){

	//Modo modificacion
	$("cajaModoModificacion").setAttribute("collapsed","false");
	$("cajaModoAlta").setAttribute("collapsed","true");

 	AutoActualizarAlbaran( IdAlbaran );
 }


function AutoActualizarAlbaran(id){

	var url = moduloAjax + "?modo=JSONCargarAlbaran&id="+id;

	ajaxCarga.open("GET",url,true);
	ajaxCarga.onreadystatechange = CargarAlbaran;
	ajaxCarga.send(null)
...
function CargarAlbaran(){
	...
	if (ajaxCarga.readyState==4) {
		...
		InterpretarDatosAlbaran(rawtext);
	}
}


function InterpretarDatosAlbaran(rawtext){
	if (!rawtext)	return;
	...
	setDatoURL2("totalalbaran","TotalImporte",obj);
	...
	setDatoURL2("datoNumalbaran","NAlbaran",obj);
	setDatoURL2_fecha("datoFecha","FechaAlbaran",obj);

	setDatoURL2("obraRealizada","ObraRealizada",obj);

	Albaran.IdAlbaran = obj.IdAlbaran;

	Albaran.IdAlbaran = obj["IdAlbaran"];
	Albaran.serie = obj["SerieAlbaran"];
	Albaran.numero = obj["NAlbaran"];
	Albaran.IdAlbaran = obj["IdCliente"];

	//TODO: cargar lineas de albaran
	AutoActualizarLineasAlbaran( obj.IdAlbaran );

	ActualizarTotalesProductos();
}

function AutoActualizarLineasAlbaran(id){

	var url = moduloAjax + "?modo=JSONCargarLineasAlbaran&id="+id;

	ajaxCarga.open("GET",url,true);
	ajaxCarga.onreadystatechange = CargarLineasAlbaran;
	ajaxCarga.send(null)
...
function CargarLineasAlbaran(){
	...
	if (ajaxCarga.readyState==4) {
		...
		InterpretarLineasAlbaran(rawtext);
	}
}

function InterpretarLineasAlbaran(rawtext){

	function ActualizaCamposProductoDesdeLineaAlbaran(obj){
		$("codigo").value = obj["IdProducto"];
		$("referencia").value = obj["Referencia"];
...
/*
	VerObj( obj );
	VerObj( obj[2] );
	0 ->2 IdAlbaranDet ->2 1 ->8 IdAlbaran ->8 2 ->0 IdProducto ->0
	...
	for( t in obj){
		...
		ActualizaCamposProductoDesdeLineaAlbaran(obj[t]);
...
function VentanaImprimirAlbaran(){
   //   VerObj(Albaran);
        AbrirVentanaImprimir("moddocumento.php?modo=albaran&id="+Albaran.IdAlbaran);
}
...
function GuardarCambios(){
	...
	cadenaEnvio += "&IdAlbaran="+ Albaran.IdAlbaran;


	var obj = new XMLHttpRequest();
	var url = moduloAjax;

	cadenaEnvio +="&modo=ModificarAlbaran";
	...
	InterpretarDatosModAlbaran(obj.responseText);

}
 

function InterpretarDatosModAlbaran(rawtext){
	...
	try {
		...
		alert("Albaran modificada con exito");
		UsarAlbaran( obj["IdAlbaran"] );
