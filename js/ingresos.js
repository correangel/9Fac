...
function onBuscar(){
	...
	url = "formingresos.php";
	resBus = EnviarData(url,dataExtra + "&modo=buscar");
...
function onEnvioDatos(isMod){
	...
	var url = "formingresos.php";
	var res = EnviarData(url,data+ "&modo=alta"+sufix);
	//alert("respuesta: "+res);
	if (res.match("=")){
	...
	switch(res.toLowerCase()){
		...
		case "ok":
			...
			if (enviarCentros){
				...
				var url = "formingresos.php";			
				resAtrib = EnviarData(url,dataExtra+ "&modo=atrib");
			...
			if (enviarActiv){
				...
				url = "formingresos.php";
				resAtrib = EnviarData(url,dataExtra+ "&modo=activ");
				cambioActiv = 0;				
			}
			//alert(resAtrib);
			if (enviarCobros){
				...
				dataExtra = preparaData( pakdata );
				url = "formingresos.php";		
				resAtrib = EnviarData(url,dataExtra+ "&modo=ingresos");
				cambioCobros = 0;
			}			
