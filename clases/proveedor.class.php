...
function CrearProveedorBreve($nombre) {
	global $UltimaInsercion;
	//Comprobamos si es correcto crear usuario con este nombre
	$sql = "SELECT IdProveedor FROM ges_proveedores WHERE (NombreComercial='$nombre') AND (Eliminado=0)";
	$row = queryrow($sql);
	if ($row and $row["IdProveedor"]>0){
		return false;//Ya existe usuario
	}
	
	$sql = "INSERT INTO ges_proveedores ( NombreComercial) VALUES ('$nombre')";
	
	if(query($sql , "alta proveedor")) {
