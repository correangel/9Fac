...
function CrearClienteBreve($nombre) {
	global $UltimaInsercion;
	//Comprobamos si es correcto crear usuario con este nombre
	$sql = "SELECT IdCliente FROM ges_clientes WHERE (NombreComercial='$nombre') AND (Eliminado=0)";
	$row = queryrow($sql);
	if ($row and $row["IdCliente"]>0){
		return false;//Ya existe usuario
	}
	
	$sql = "INSERT INTO ges_clientes ( NombreComercial) VALUES ('$nombre')";
