...
include("inc/modpedidos.inc.php");

StartXul("modpedido",false,false,true);
...
<popupset>
  <popup id="accionesLista" class="media">
	   <menuitem class="menuitem-iconic" image="img/addcart16.gif" label="Quitar"  oncommand="quitarElementoPedido(this)"/>
  </popup>
	...
	<toolbar id="a-toolbar" >
		...
		<toolbarbutton image="img/busca1.gif" label="Buscar" oncommand="Accion_BuscarPedido()"/>
	</toolbar>


<groupbox flex="1">
<caption id="captionclick" >
<description>Cabecera de pedidos</description>
<vbox><spacer flex="1"/>
...
<vbox  flex="1">
<hbox align="center">
<label class="labelGeneral">Nº Pedido:</label>
<textbox disabled="true" value="" class="datoNumpedido" id="datoNumpedido" flex="0"/>
<toolbarbutton  image="img/edit.png"  oncommand="habilitarEdicion('datoNumpedido')" collapsed="false"/>
<toolbarbutton  image="img/addnew.gif" oncommand="CargarSiguientesDatosDePedido()"  zoncommand="AbrirVentanaNumfactura()" collapsed="false"/>
<spacer flex="1"/>
</hbox>

<hbox align="center">
<label class="labelGeneral">Fecha Pedido:</label>
...
<vbox>
<label value="Total Pedido" style="font-weight: bold"/>
<textbox id="totalpedido" value="" style="text-align: right;color:gray" readonly="true"/>
</vbox>

</hbox>
</groupbox>
<hbox  id="cajaModoModificacion" collapsed="true">
    <button image="img/printicon.png" label="Imprimir" oncommand="VentanaImprimirPedido()"/>
...
<hbox id="cajaModoAlta">
    <spacer flex="1"/>
    <button image="img/enviar.png" label="Crear pedido" oncommand="EnviarPedido()"/>
    <button image="img/button_cancel.png" label="Cancelar pedido" oncommand="CancelarPedido()"/>
</hbox>
...
function toggleDesde(){
	...
	if ( !this.colapsado ){
		...
		$("captionclick").setAttribute("label","Cabecera de pedido ∆");
		this.colapsado = true;
	} else {
...
Pedido.numero = 3;
Pedido.serie = "A";
Pedido.numeropedido= "A-3";
Pedido.fecha = "<?php echo date("d-m-Y") ?>";
Pedido.IdCliente = 0;
Pedido.IdFactura = 0;

]]></script>
<script type="application/x-javascript" src="js/modpedidos.js?ver=1/<?php echo rand(0,99999999); ?>"/>
